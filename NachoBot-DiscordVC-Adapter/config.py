...
def _resolve_stt_config_from_nachobot(nachobot_config_path: Path) -> STTConfig:
    ...
    try:
        data = toml.load(nachobot_config_path)
        ...
        model_def = next((m for m in models if m.get("model_identifier") == target_model or m.get("name") == target_model), None)
        ...
        provider_def = next((p for p in providers if p.get("name") == provider_name), None)
