...
class VoiceCog(commands.Cog):
    ...
    @discord.slash_command(name="join-vc", description="Join your voice channel (Server Only)")
    ...
    @discord.slash_command(name="leave-vc", description="Leave the voice channel (Server Only)")
...
class NachoDiscordBot(discord.Bot):
    def __init__(self, config: AdapterConfig, voice_handler: VoiceHandler, logger: logging.Logger):
        super().__init__(intents=intents)
        self.adapter_config = config
        self.voice_handler = voice_handler
        self.logger = logger
        self.speech_callback: Optional[Callable[[int, int, str], None]] = None # guild_id, user_id, text
    ...
    async def _on_sink_callback(self, user_id: int, pcm_data: bytes, guild_id: int):
        ...
        if text and self.speech_callback:
            # Notify adapter
            await self.speech_callback(guild_id, user_id, text)
