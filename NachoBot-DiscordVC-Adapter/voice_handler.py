...
from config import AdapterConfig, VoiceConfig, STTConfig
...
class SilenceDetectingSink(Sink):
    ...
    """
    def __init__(self, filters=None, callback: Callable = None, config: VoiceConfig = None):
        if filters is None:
        ...
        self.vad_threshold = 100   # RMS threshold (adjustable)
    ...
    def write(self, data, user):
        ...
        if is_speech:
            ...
            if not self.is_speaking[user]:
                ...
                logging.getLogger("VoiceHandler").debug(f"User {user} started speaking (RMS: {rms})")
        ...
        while not self._stopped:
            ...
            for user in users:
                ...
                if now - last_speech > self.silence_threshold:
                    ...
                    length_seconds = len(data) / (DISCORD_SAMPLE_RATE * DISCORD_CHANNELS * DISCORD_WIDTH)
                    
                    if length_seconds >= self.min_speech_duration:
                        # Flush and callback
                        logging.getLogger("VoiceHandler").debug(f"Speech detected from {user}: {length_seconds:.2f}s")
                        if self.callback:
...
class VoiceHandler:
    ...
    async def process_audio(self, user_id: int, pcm_data: bytes) -> Optional[str]:
        ...
        if len(pcm_data) % (DISCORD_CHANNELS * DISCORD_WIDTH) != 0:
            # Padding? or Trimming
            trim_len = len(pcm_data) - (len(pcm_data) % (DISCORD_CHANNELS * DISCORD_WIDTH))
