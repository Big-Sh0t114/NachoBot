        ...
        if cached_result is not None:
            ...
            if result.success:
                ...
                should_reconnect = (
                    ...
                    "target closed" in error_lower or "session closed" in error_lower
                ) or (
                    "not connected" in error_lower
                )

                if should_reconnect:
                    logger.warning(f"检测到 MCP 服务器 {self._mcp_server_name} 可能断开 ({result.error})，尝试自动重连...")
                    # 异步触发重连，不等待
