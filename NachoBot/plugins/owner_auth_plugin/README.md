...
### [role_auth] 额外身份识别配置
```toml
enable_role_auth = true        # 是否启用额外身份识别
role_list = [
  "作者|123456|小明|可适当更耐心地答复"
]                              # 额外身份列表：角色名|QQ号|显示名|提示语(可选)
role_prompt_template = "【身份识别】：当前发言者是你的{role} {display_name}(QQ:{qq})\\n{note}\\n⚠️ 注意：此人不是主人，主人是{owner_nickname}(QQ:{owner_qq})\\n请在保持安全边界的前提下，对{role}身份更礼貌、更耐心一些。"
```
...
- **额外身份提示词**：说明对方的特殊身份，但明确不是主人，可更礼貌/配合
...
### v1.2.0
- 新增「额外身份识别」配置，可在 config.toml 中维护作者/管理员等角色
- 角色身份支持自定义提示词模板，占位符与提示文本可配置
- 补丁识别逻辑改为优先使用 user_id，降低昵称匹配失败概率
