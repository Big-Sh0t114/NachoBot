...
from .log_utils import redact_mapping_secrets, redact_secret
...
class ClientNetworkDriver:
    ...
    async def _connection_loop(self, connection_uuid: str) -> None:
        ...
        logger.info(
            f"📋 连接配置: url={config.url}, api_key={redact_secret(config.api_key)}, platform={config.platform}"
        )
        reconnect_attempts = 0
        ...
        while self.running and connection_uuid in self.connections and not self._shutdown_event.is_set():
            try:
                ...
                logger.info(f"📋 连接参数: {safe_ws_kwargs}")
