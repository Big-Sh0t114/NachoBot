...
ASCII_ART = r"""
     ...
     X XXXX                    XXXX X            _   _____   ________  ______ 
    XXXX                          XXXX          / | / /   | / ____/ / / / __ \
  XXX                                XXX       /  |/ / /| |/ /   / /_/ / / / /
 XX                                    XX     / /|  / ___ / /___/ __  / /_/ / 
 X     XXXXXX     XXXXXX     XXXXXX     X    /_/ |_/_/__|_\____/_/_/_/\____/  
 X     X    X     X    X     X    X     X            / __ )/ __ \/_  __/      
 X     XX  XX     XX  XX     XX  XX     X           / __  / / / / / /         
 X      XXXX       XXXX       XXXX      X          / /_/ / /_/ / / /          
 XX                                    XX         /_____/\____/ /_/           
   XX                                XX      
    XXXX                          XXXX       
       XXXXXX                XXXXXX          
...
def print_startup_banner():
    ...
    art_to_print = ASCII_ART_RARE if random.random() < 0.1 else ASCII_ART
    print(Fore.WHITE + art_to_print + Style.RESET_ALL)
...
async def graceful_shutdown(main_system: MainSystem | None = None):  # sourcery skip: use-named-expression
    try:
        ...
        logger.info("NachoBot优雅关闭完成")

        # 关闭日志系统，释放文件句柄
        shutdown_logging()

    except Exception as e:
        logger.error(f"NachoBot关闭失败: {e}", exc_info=True)
