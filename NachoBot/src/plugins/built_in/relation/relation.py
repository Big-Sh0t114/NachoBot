...
class BuildRelationAction(BaseAction):
    ...
    async def execute(self) -> Tuple[bool, str]:
        ...
        try:
            ...
            if not success or not category or not category.strip():
                fallback_category = "其他"
                logger.warning(
                    f"{self.log_prefix} LLM未给出分类，使用兜底分类: {fallback_category}，success={success}, raw={category!r}"
                )
                ...
                try:
                    ...
                    fallback_category = "其他"
                    logger.warning(
                        f"{self.log_prefix} 解析分类结果失败: {e}, raw={category!r}，使用兜底分类: {fallback_category}"
                    )
                    category_data = {"category": fallback_category}
            ...
            if not success or not update_memory or not update_memory.strip():
                logger.warning(
                    f"{self.log_prefix} LLM未返回记忆更新结果，直接添加新记忆: success={success}, raw={update_memory!r}"
                )
                person.memory_points.append(f"{category}:{impression}:1.0")
                person.sync_to_database()
                return True, "记忆更新无结果，已直接添加新记忆"
