...
class PrivateReplyer:
    ...
    async def build_prompt_reply_context(
        ...
        guarded_dialogue_prompt, long_history_injection, _ = guard_user_content(dialogue_prompt, None)
        if long_history_injection:
            dialogue_prompt = guarded_dialogue_prompt
        ...
        if chosen_actions:
            for action in chosen_actions:
                ...
                if action_type == "reply":
                    ...
                    if planner_question_text is None and hasattr(action, "action_data") and isinstance(action.action_data, dict):
                        planner_question_text = action.action_data.get("question")
        ...
        if advanced_on and global_config.advanced.block_tts_when_on:
            tts_language_prompt = ""
            tts_selected = False
        ...
        if advanced_on and global_config.advanced.prompt:
        ...
        if advanced_on:
            # ADVANCED_PROMPT_INJECTION: 高级模式下改用自定义破甲提示词并保留注入防护
    ...
    async def build_prompt_rewrite_context(
        ...
        if advanced_on:
            # ADVANCED_PROMPT_INJECTION (rewrite): 高级模式使用自定义提示 + 注入防护
            base_prompt = global_config.advanced.prompt.strip()
            moderation_prompt_block = f"{base_prompt}\n{build_guardrail_instruction(injection_detected)}".strip()
        else:
