...
class AdvancedIdleTimeoutTask(AsyncTask):
    ...
    def __init__(self):
        ...
        group_timeout_minutes = max(1, int(getattr(cfg, "group_idle_timeout_minutes", 60) or 0))
    ...
    async def run(self):
        ...
        for stream in list(chat_manager.streams.values()):
            is_group = bool(stream.group_info)
            if not advanced_manager.is_on(stream):
                continue

            idle_seconds = now - getattr(stream, "last_active_time", now)
            if is_group:
                if idle_seconds < self._group_timeout_seconds:
                    continue
                group_id = getattr(stream.group_info, "group_id", None)
                if not group_id:
                    continue
                advanced_manager.set_group_state(stream.platform, str(group_id), False)
                logger.info(
                    f"[AdvancedIdle] group={group_id}, stream={stream.stream_id} idle_for={int(idle_seconds)}s -> turned off"
                )
                try:
                    await send_api.text_to_stream(self._group_notice, stream.stream_id)
