...
class PersonProfileService:
    ...
    def resolve_person_id(identifier: str) -> str:
        ...
        try:
            record = PersonInfo.select(PersonInfo.person_id).where(PersonInfo.person_id == key).first()
            if record:
                return str(record.person_id)

            record = PersonInfo.select(PersonInfo.person_id).where(
                (PersonInfo.person_name == key) | (PersonInfo.user_nickname == key)
            ).first()
            if record:
                return str(record.person_id)

            record = PersonInfo.select(PersonInfo.person_id).where(
                PersonInfo.group_cardname.contains(key)
            ).first()
            if record:
                return str(record.person_id)
        except Exception as e:
    ...
    def get_person_aliases(self, person_id: str) -> Tuple[List[str], str, List[str]]:
        ...
        try:
            record = PersonInfo.select().where(PersonInfo.person_id == person_id).first()
            if not record:
