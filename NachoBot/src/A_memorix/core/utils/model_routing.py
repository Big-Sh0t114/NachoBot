...
def build_single_model_task(model_name: str, template: Any) -> Any:
    ...
    kwargs = {
        ...
        "max_tokens": getattr(template, "max_tokens", 1024),
        "temperature": getattr(template, "temperature", 0.5),
    }
    for attr in ("slow_threshold", "selection_strategy", "hard_timeout", "timeout"):
        if hasattr(template, attr):
            kwargs[attr] = getattr(template, attr)
...
async def generate_with_resolved_model(
    ...
    *,
    ...
    client._orchestrator.model_usage = {model.selected_model_name: (0, 0, 0, 0.0)}
