...
from src.config.official_configs import AMemorixConfig
from ._compat import config_manager
if not hasattr(Config, "a_memorix"):
    setattr(Config, "a_memorix", property(lambda self: config_manager.a_memorix))
config_fields = getattr(AMemorixConfig, "__dataclass_fields__", {}) or {}
if "integration" not in config_fields:
    class DummyIntegration:
        def __getattr__(self, name):
            return False
    setattr(AMemorixConfig, "integration", property(lambda self: DummyIntegration()))
if "cross_chat_memory" not in config_fields:
    class DummyCrossChatMemory:
        enabled = False
    ...
    setattr(AMemorixConfig, "cross_chat_memory", property(lambda self: DummyCrossChatMemory()))
...
class AMemorixHostService:
    ...
    async def invoke(self, component_name: str, args: Dict[str, Any] | None = None, *, timeout_ms: int = 30000) -> Any:
        ...
        if component_name == "memory_stats":
            kernel = self._kernel
            if kernel is None or not kernel.is_runtime_ready():
                return {
                    "success": True,
                    "initialized": False,
                    "ready": False,
                    "note": "A_Memorix 已启用，但核心尚未初始化。打开长期记忆页面时不会再强制启动内核。",
                }
