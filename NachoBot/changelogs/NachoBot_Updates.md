9/13
    更换本地情感分类器模型，使用专业的情感分类模型
9/10
    focus模式允许私聊切换携带元信息，仍禁止携带上下文
    全面升级沙盒系统，使用真正的agent多轮迭代架构
9/6
    添加Multimodal Runtime管理多模态适配器的环境使用
9/2
...
8/17
    修复新版Napcat适配器部分类型信息id为空会导致解析失败的问题
    最小化感情分类器所需下载的模型资源
8/16
    更新音乐库插件音频目录位置
    更新多模态适配器cpu推理依赖
8/15
    修复WebUI配置页面显示脱敏密文的bug
    修复WebUI核心未就绪状态下无法发送确认信息的bug
    修复8070端口未绑定的问题
8/14
    核心及各适配器新增令牌认证，凭据通过环境变量注入
    适配器连接支持自定义CA证书与跳过TLS校验
    WebUI新增登录认证，配置编辑密钥脱敏
    Docker端口默认仅绑定本机，新增健康检查
    优化聊天循环稳定性，防止运行中被回收
    记忆系统新增消息游标持久化，重启后不丢进度
    新增MCP工具执行器与能力路由
    artwork插件迁移至外部插件目录
    Live2D适配器新增模型参数适配
    Napcat适配器新增监听地址自动解析
    TTS运行时下载新增进度显示
    启动脚本新增Playwright浏览器检查
    更新各适配器及插件文档
8/9
    重构TTS推理端，高度集成进本项目，不依赖外部客户端
    新增launchbot_potato启动方式，不启动任何本地模型
    解耦核心中的bilibili二阶段搜索链路
    更新核心及各适配器docker文件
8/8
    修改本地模型的启动顺序为优先缓存启动
8/7
    修复WebUI Chat中多段回复概率被吞的问题
    修复思考时的三点动画会在切换页面时消失的问题
8/6
    移除VoxCPM的denoise字段
8/5
...
7/30
    解耦核心中bilibili相关逻辑
    更新多模态适配器ASR模型，通用语音适配器ASR合并入多模态适配器
7/29
    优化WebUI音乐可视化表现
    WebUI接入TTS
7/28
    ...
    依赖更新
7/27
    重写Napcat适配器以及多模态适配器的库导入，移除重复库
7/24
    优化Live2D适配器的启动逻辑&补全缺失依赖
7/23
    优化仓库大小，重写WebUI音乐播放器逻辑
7/22
    分离Bilibili侧Live2D部分，使用独立适配器运行
    重命名TTS适配器至多模态适配器，分离VLM与ASR
    Bilibili新增活动窗口跟踪捕获
    优化WebUI素材储存占用
7/19
    移除联网搜索对模型能力的硬限制
...
7/16
    添加Messenger插件用户确认过滤
    Bilibili视频搬运插件添加时长过滤配置项
    focus模式开发
    用户名片解析更新
7/14
    Napcat适配器新增对各种卡片信息的解析
    bilibili视频解析插件同步支持对视频卡片解析
7/11
    修复数据库清理问题
7/9
    Messenger插件添加用户名单过滤
