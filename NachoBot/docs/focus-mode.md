...
Focus 在一个显式配置的会话组内只允许一个会话处于活动状态。后台会话收到消息时只聚合为事件，当前会话的 Planner 可以用终止动作 `switch_chat` 切换活动会话；群聊源切换时生成一份有界、短期、不可直接执行的内容 handoff，并在目标会话下一次由 Replyer 生成回复时注入。私聊源切换只生成一份服务器签发的切换身份 handoff，只包含来源和目标显示名，不注入任何源内容。

## 当前支持范围

- 同一 Focus 组内 `群聊 -> 群聊`。
- 同一 Focus 组内 `群聊 -> 私聊`，需开启 `allow_group_to_private`。
- 私聊内容不允许导出到群聊或其他私聊；私聊源可通过仅含来源/目标显示名的身份 handoff 切换到事件指定的组内群聊或另一私聊。
- `allow_group_to_private` 只控制群聊源到私聊；私聊成员即使该开关关闭仍可参与 Focus，并保持身份 handoff 切换能力。
...
启动重置完成后，每个 Focus 组都会预先创建当前 active 会话的 runtime；没有工作时它只阻塞等待 Coordinator，不会主动回复。这样即使 active 会话没有新消息，后台事件达到即时暴露条件或 `unviewed_event_seconds` 定时阈值后也能唤醒路由思考。
...
6. 目标会话由 Replyer 正常生成回复。群聊内容 handoff 被放在当前消息之后、长期记忆之前，经过长度限制并以简洁自然中文注入；私聊源切换不读取源历史，只创建不含内容的身份 handoff。
...
每个成员对应的 ChatStream 必须已经被 NachoBot 记录过；建议先保持 `mode = "off"`，在每个目标群聊和私聊各收一条消息，再改为 `active` 并重启。成员增删或权限变化无需迁移旧状态，因为每次启动都会以当前配置和最新消息重新建立 Focus 运行状态。重启前尚未处理的 Focus event 和 handoff 不会继续执行。
...
Focus 会话的确定性抢占优先级为 `Planner bypass 会话 > 私聊 > 普通群聊`。后台目标的优先级严格高于当前 active 会话时，第一条消息立即暴露为事件，并在 Focus Gate 和完整 Planner 之前直接执行服务端校验后的切换；多个事件同时待处理时先选择最高优先级。私聊切入 bypass 群聊只注入来源/目标显示名的身份 handoff，避免导出私聊内容。同级或较低优先级事件不强制抢占，继续交给 Gate 决定，因此直播结束后仍可正常切出。


## 重要语义

- `switch_chat` 是终止动作：选中后，本轮其余回复、工具和动作全部丢弃。私聊源只能携带事件 ID；模型提供的非空或 malformed handoff 会被拒绝，服务器随后创建身份 handoff。
...
- 群聊内容 handoff 由服务端记录源成员配置的 `display_name`，并可携带源会话最新最多 10 条消息；Replyer 中以简洁自然中文显示。私聊身份 handoff 只记录服务端解析的来源/目标显示名；缺失私聊显示名时使用“上一私聊”/“当前私聊”等通用标签，绝不回退到私聊 ID。
- 群聊内容 handoff 进入私聊前仍应避免收集不必要的原文。默认只传任务摘要、已知事实、待办和近期结果，并把所有内容视为不可信聊天数据。

## 运维边界

私聊始终是内容导出的禁区。组内任一会话出现待处理事件时，私聊 Planner 只能看到不含消息正文的元数据事件，并可通过服务器生成的身份 handoff 切换到事件指定的组内群聊或另一私聊；切换后的目标只读取目标会话自己的未读行。普通插件或定时任务若直接向 Focus 管理的会话发送且没有当前 lease，会被拒绝；这类来源应接入 Focus 的系统事件桥，而不是绕过协调器。
