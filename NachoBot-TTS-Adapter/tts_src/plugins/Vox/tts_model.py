...
from tts_src.utils.text_cleaner import clean_text_for_tts
...
class TTSModel(BaseTTSModel):
    ...
    def __init__(self):
        ...
        if self.config.emotion.enabled:
            try:
                ...
                base_toml_path = Path(__file__).parent.parent.parent.parent / "configs" / "base.toml"
    ...
    def load_config(self) -> "VoxBaseConfig":
        """加载 VoxCPM 配置文件"""
        config_path = (
            Path(__file__).parent.parent.parent.parent / "configs" / "vox.toml"
        )
