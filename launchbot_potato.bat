...
set "PYTHONPATH="
set "PYTHONHOME="
title Launch NachoBot Potato (Core Text Profile)
set "FINAL_RC=0"
set "ROOT=%~dp0"
set "NACHOBOT_FFMPEG_DIR=%ROOT%.runtime\ffmpeg"
set "NACHOBOT_RUNTIME_PROFILE=potato"
...
echo.
echo ===== Start Main Bot Component (POTATO: Core text profile + QQ adapter) =====
...
if exist "%NACHOBOT_DIR%\%NACHOBOT_MAIN%" (
  echo --- Start NachoBot...
  start "NachoBot" /D "%NACHOBOT_DIR%" cmd /k "set HOST=127.0.0.1 && set PORT=%NACHOBOT_PORT% && set NACHOBOT_RUNTIME_PROFILE=%NACHOBOT_RUNTIME_PROFILE% && %PYTHON_CMD% %NACHOBOT_MAIN%"
...
echo.
echo Startup complete. NachoBot Core and the selected QQ adapter are running in POTATO text profile; no local Multimodal service was started.
