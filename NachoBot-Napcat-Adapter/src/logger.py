...
logger.add(
    ...
    filter=lambda record: "name" not in record["extra"] or record["extra"].get("name") != "ncnk_message",
)
logger.add(
    sys.stderr,
    level="INFO",
...
# 创建样式不同的 logger
custom_logger = logger.bind(name="ncnk_message")
logger = logger.bind(name="Nachobot-Napcat-Adapter")
