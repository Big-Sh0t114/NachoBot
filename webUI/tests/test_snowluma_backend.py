...
def _prepare_runtime(root: Path, *, version: str = "1.14.17") -> None:
    for relative in snowluma.REQUIRED_SNOWLUMA_RUNTIME_FILES:
        _write(root, f"SnowLuma/{relative}", b"runtime")
    _write(
        root,
        "SnowLuma/package.json",
...
def test_installation_status_has_official_url_and_missing_components(tmp_path: Path) -> None:
    ...
    assert any(item.startswith("SnowLuma/") for item in status["missing"])
...
def test_synchronize_writes_real_three_file_schema_and_preserves_generated_at(
    ...
    onebot = json.loads((tmp_path / "SnowLuma/config/onebot_123456.json").read_text(encoding="utf-8"))
    ...
    webui_path = tmp_path / "SnowLuma/config/webui.json"
...
def test_synchronize_rejects_running_and_totp_or_malformed_auth(
    ...
    webui = tmp_path / "SnowLuma/config/webui.json"
    _write(
        tmp_path,
        "SnowLuma/config/webui.json",
        json.dumps(
            {
                "passwordHash": "0" * 128,
                "passwordSalt": "0" * 32,
...
def test_synchronize_rolls_back_target_that_throws_after_write(
    ...
    _prepare_runtime(tmp_path)
    _patch_safe_sync(monkeypatch)
    target = tmp_path / "SnowLuma/config/onebot_123456.json"
...
def test_select_qq_adapter_preserves_comments_and_rebuilds_registry(
    ...
    with mock.patch("process_manager._register_services") as register:
