...
class ModelClient:
    ...
    async def call_replyer(self, prompt: str) -> Optional[str]:
        ...
        Returns the model's response text or None if failed.
        """
        realtime_config = self._task_configs.get("realtime_replyer")
        if realtime_config and getattr(realtime_config, "model_list", None):
            return await self._call_task_model("realtime_replyer", prompt)
