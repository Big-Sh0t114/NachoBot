"""Bilibili live V2 protocol models.

Field mappings are aligned with the current blivedm web protocol implementation.
The models intentionally live inside the adapter so V2 decoding doesn't depend on
blivedm itself.
"""
...
class InteractWordV2UserInfo(pb_msg.BaseMessage):
    base: Annotated[InteractWordV2UserBaseInfo, pb_anno.Field(2)] = dataclasses.field(
        default_factory=InteractWordV2UserBaseInfo
    )
...
class InteractWordV2(pb_msg.BaseMessage):
    ...
    # Bilibili currently exposes the user's 大航海 type here.
    # 0=none, 1=总督, 2=提督, 3=舰长.
    privilege_type: Annotated[int, pb_anno.Field(16)] = 0
    uinfo: Annotated[InteractWordV2UserInfo, pb_anno.Field(22)] = dataclasses.field(
        default_factory=InteractWordV2UserInfo
    )

    @classmethod
    def from_base64(cls, value: str) -> "InteractWordV2":
        return cls.loads(base64.b64decode(value))
...
class SendGiftBroadcast(pb_msg.BaseMessage):
    ...
    medal_info: Annotated[SendGiftV2MedalInfo, pb_anno.Field(8)] = dataclasses.field(
        default_factory=SendGiftV2MedalInfo
    )
    blind_gift: Annotated[SendGiftV2BlindGift, pb_anno.Field(9)] = dataclasses.field(
        default_factory=SendGiftV2BlindGift
    )
    gift_list: Annotated[List[SendGiftV2GiftItem], pb_anno.Field(10)] = dataclasses.field(
        default_factory=list
    )

    @classmethod
    def from_base64(cls, value: str) -> "SendGiftBroadcast":
        return cls.loads(base64.b64decode(value))
