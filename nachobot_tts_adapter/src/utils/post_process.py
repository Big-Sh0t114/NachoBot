...
def simulate_telephone_voice(audio_bytes) -> bytes:
    ...
    def soft_clip(x, threshold=0.95):
        ...
        y[idx] = x[idx]
        y[~idx] = np.sign(x[~idx]) * (
            threshold + (1 - threshold) * np.tanh((np.abs(x[~idx]) - threshold) / (1 - threshold))
        )
        return y
    ...
    if samp_width == 1:  # 8-bit
        processed_int = (processed_audio * 127).astype(np.int8)
    elif samp_width == 2:  # 16-bit
        processed_int = (processed_audio * 32767).astype(np.int16)
    elif samp_width == 4:  # 32-bit
        processed_int = (processed_audio * 2147483647).astype(np.int32)
    else:
        # 默认使用16-bit
        processed_int = (processed_audio * 32767).astype(np.int16)
