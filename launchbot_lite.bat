...
set "NACHOBOT_RUNTIME_PROFILE=lite"
...
set "NACHOBOT_TTS_ENDPOINT=http://!CONNECT_HOST_ADAPTER!:9880"
...
:TTS_END
endlocal & set "NACHOBOT_TTS_ENDPOINT=http://%CONNECT_HOST_ADAPTER%:9880" & exit /b %TTS_RC%
...
if exist "%NACHOBOT_DIR%\%NACHOBOT_MAIN%" (
  echo --- Start NachoBot...
  start "NachoBot" /D "%NACHOBOT_DIR%" cmd /k "set HOST=127.0.0.1 && set PORT=%NACHOBOT_PORT% && set NACHOBOT_RUNTIME_PROFILE=%NACHOBOT_RUNTIME_PROFILE% && set NACHOBOT_TTS_ENDPOINT=%NACHOBOT_TTS_ENDPOINT% && %PYTHON_CMD% %NACHOBOT_MAIN%"
